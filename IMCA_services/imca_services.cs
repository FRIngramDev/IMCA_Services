...
namespace IMCA_Services
{
    public partial class imca_services : ServiceBase
    {
        ...
        private string start_terminal_sessions = "";
        ...
        public class TS_Session
        {
            ...
            public Boolean rdp_connection_in_progress = false;
            ...
            private static extern IntPtr GetClassName(IntPtr hwnd, StringBuilder lpChildClassName, IntPtr nMaxCount);

            [DllImport("user32.dll")]
            private static extern long GetWindowText(IntPtr hwnd, StringBuilder lpString, IntPtr cch);


            [DllImport("user32")]
            private static extern long GetWindow(IntPtr hwnd, IntPtr wCmd);
            ...
            public string check_session_status(string session_name, string service_path)
            {
                ...
                if (WTSEnumerateSessions((IntPtr)WTS_CURRENT_SERVER_HANDLE, 0, 1, out pSessions, out nSessions))
                {
                    ...
                    for (int Index = 0; Index < nSessions; Index++)
                    {
                        ...
                        if (sUserName.ToUpper().Contains(session_name) == true)
                        {
                ...
                {
                    WriteToFile("Need additionnal access for WTSEnumerateSessions : ", service_path);
                }

                return bol;
            }

            public void WriteToFile(string message, string service_path, string logs_folder = "logs")
            {
                ...
                string filepath = logs_folder + "\\IMCA_" + service_name.ToUpper() + "_" + DateTime.Now.Date.ToString("dd_MM_yyyy") + ".txt";
            ...
            public void CreateTerminalSession(string server, string user, string domain, string password)
            {
                void ProcessTaskThread()
                {
                    form = new Form();
                    
                    var rdpConnection = new AxMSTSCLib.AxMsRdpClient6NotSafeForScripting();
                    form.Text = "IMCA Terminal Session Form - " + user.ToUpper();
                    ...
                    form.Load += (sender, args) =>
                    {
                        ...
                        //rdpConnection.AdvancedSettings.allowBackgroundInput = 1;

                        rdpConnection.OnDisconnected += RdpConnectionOnOnDisconnected;
                        rdpConnection.OnLoginComplete += RdpConnectionOnOnLoginComplete;
                        rdpConnection.OnLogonError += RdpConnectionOnOnLogonError;
                        rdpConnection.OnAuthenticationWarningDisplayed += new EventHandler((s, e) => OnAuthenticationWarningDisplayed_light(s, e, password));
                        //rdpConnection.OnConnected += new EventHandler((s, e) => LoginComplete(s, e));
                        
                        rdp_connection_in_progress = true;
                ...
                WriteToFile("                 Thread(ProcessTaskThread : ", service_path);
                ...
                int i = 1; 
                while (rdpClientThread.IsAlive && i <= 60) // We wait 60 secondes max
                {
                    Task.Delay(1000).GetAwaiter().GetResult();
                    System.Windows.Forms.Application.DoEvents();
                    WriteToFile("                 rdpClientThread.IsAlive : " +  i.ToString() + " seconde(s)", service_path);
                    i = i + 1;
                }

                WriteToFile("              After rdpClientThread.IsAlive", service_path);

            }
            private void LoginComplete(object sender, EventArgs e)
            {
                var rdpSession = (AxMSTSCLib.AxMsTscAxNotSafeForScripting)sender;

                MsRdpClient8NotSafeForScripting secured = (MsRdpClient8NotSafeForScripting)rdpSession.GetOcx();
                IMsRdpClientNonScriptable rdpSession_NonScriptable = (IMsRdpClientNonScriptable)rdpSession.GetOcx();
                ...
                System.Drawing.Point loc = rdpSession.PointToScreen(System.Drawing.Point.Empty);

                var cnt = new HashSet<System.Drawing.Color>();
                Dictionary<System.Drawing.Color, int> nb_cnt = new Dictionary<System.Drawing.Color, int>();

                var duplicates = new HashSet<System.Drawing.Color>();

                //            string filename = @"C:\temp\rdp.png";
                System.Drawing.Size bitmapSize = new System.Drawing.Size(rdpSession.DesktopWidth, rdpSession.DesktopHeight - 16);
                ...
                var nb_pixels = rdpSession.DesktopWidth * (rdpSession.DesktopHeight - 16);
                Boolean bool_warning = false;

                foreach (KeyValuePair<System.Drawing.Color, int> percent in nb_cnt.OrderByDescending(key => key.Value).Take(1)) // Take the most popular color
                {
                    if (System.Math.Round((decimal)percent.Value / nb_pixels * 100, 2) > (decimal)80.0) // If we have more than 80% the same color we consider to be in front of the Warning logon screen
                    {
                        bool_warning = true;
                        break;
                    }
                ...
                if (bool_warning)
                {
                    ...
                    // Retour Chariot
                    keyData[0] = MapVirtualKey(VK_ENTER, 0);

                    try
                    {
                        rdpSession.Select();
                        rdpSession_NonScriptable.SendKeys(1, ref keyUp[0], ref keyData[0]);
                    }
                    catch (Exception)
                    {
                        throw;
                ...
                rdp_connection_in_progress = false;
            }

            

            private void OnAuthenticationWarningDisplayed_light(object sender, EventArgs e, string password)
            {
                //var rdpSession = (AxMsRdpClient6NotSafeForScripting)sender;
                //IMsRdpClientNonScriptable2 rdpSession_NonScriptable = (IMsRdpClientNonScriptable2)rdpSession.GetOcx();
                WriteToFile("                 OnAuthenticationWarningDisplayed", service_path);
            ...
            private void RdpConnectionOnOnLoginComplete(object sender, EventArgs e)
            {
                ...
                var rdpSession = (AxMsRdpClient6NotSafeForScripting)sender;
                WriteToFile("                 After 20 secondes we disconnect the session", service_path);
        ...
        public class JSON_file_config_services
        {
            ...
            public string START_TERMINAL_SESSIONS { get; set; }
        ...
        private void Open_Terminal_Sessions()
        {

            string sql;
            List<string> list_server = new List<string>();

            using (var con_sql = new SqlConnection(sql_con))
            {
                con_sql.Open();

                using (var cmd_open_rdp_session = new SqlCommand())
                {
                    cmd_open_rdp_session.Connection = con_sql;

                    string nom_machine = System.Environment.MachineName.ToUpper().Trim();
                    string fqhn = System.Net.Dns.GetHostEntry(nom_machine).HostName.ToUpper().Trim();

                    sql = @"SELECT DISTINCT PARSENAME(replace([PCM_TAB_IMCA_LOGON].[SESSIONID],'\','.'),1) as user_name,[SESSIONID_INFORMATION] as [info] from [PCM_TAB_IMCA_LOGON] INNER JOIN [PCM_TAB_IMCA_LOGON_INFO] on rtrim(upper(PARSENAME(replace([PCM_TAB_IMCA_LOGON].[SESSIONID],'\','.'),1)))=rtrim(upper([PCM_TAB_IMCA_LOGON_INFO].[SESSIONID])) ";
                    sql += @" where USER_TYPE in ('IMCA','LMC','MISC','FR_LOCAL_APPS') and [PCM_TAB_IMCA_LOGON].[SESSIONID]<>'---' ";
                    sql += @" and upper(PARSENAME(replace([PCM_TAB_IMCA_LOGON].[SESSIONID],'\','.'),2))='" + nom_machine.ToUpper().Trim() + "'";
                    sql += @" ORDER BY PARSENAME(replace([PCM_TAB_IMCA_LOGON].[SESSIONID],'\','.'),1)";

                    cmd_open_rdp_session.CommandText = sql;
                    var dr_rdp_session = cmd_open_rdp_session.ExecuteReader();

                    if (dr_rdp_session.HasRows)
                    {
                        WriteToFile("   Start Terminal Session(s)               : " + nom_machine + " - Fully Qualified Domain Name : " + fqhn);

                        while (dr_rdp_session.Read())
                        {
                            // We open the Terminal Session

                            WriteToFile("   Check Terminal Session " + dr_rdp_session["user_name"].ToString().ToUpper() + "         : ");

                            string is_connected = "0";

                            try
                            {
                                var sess = new TS_Session();
                                sess.service_path = service_path;
                                sess.service_name = session_name;

                                is_connected = sess.check_session_status(dr_rdp_session["user_name"].ToString().ToUpper(), service_path);
                                switch (is_connected)
                                {
                                    case "0":
                                        WriteToFile("      ********   Start Terminal Session            ********");
                                        sess.CreateTerminalSession(nom_machine, dr_rdp_session["user_name"].ToString().ToUpper(), "corporate.ingrammicro.com", Decrypt(dr_rdp_session["info"].ToString()));
                                        int i = 0;
                                        while (sess.rdp_connection_in_progress == true && i <= 20)
                                        {
                                        ...
                                        if (i > 20)
                                        {
                                            WriteToFile("      ********   Unable to start Terminal Session            ********");
                                            // Pas eu de validation de la fenetre de securité au bout de 10 secondes (20 * 500 ms), on quitte la boucle sans etablir les autres connections du group de serveur
                                            break;
                                        }
                                    ...
                                    case "2":
                                        WriteToFile("      ********    Waiting for a client to connect  ********");
                                        break;

                                    case "3":
                                        WriteToFile("      ********    User already logged on           ********");
                                        break;

                                    case "4":
                                        WriteToFile("      ********    Logged on without client         ********");
                                        break;

                                    default:
                                        WriteToFile("      ********    Found with a unknown status (" + is_connected + ")  ********");
                                        break;
                                }
                            }
                            ...
                            {
                                WriteToFile("      ********   check_session_status Failed : " + ex.Message + "   ********");
                                throw;
                            }
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                start_terminal_sessions = param.START_TERMINAL_SESSIONS;
                ...
                if (start_terminal_sessions is null)
                {
                    start_terminal_sessions = "FALSE";
                }
                ...
                if (start_terminal_sessions.Trim().ToUpper() == "TRUE")
                {
                    Open_Terminal_Sessions();
                }
